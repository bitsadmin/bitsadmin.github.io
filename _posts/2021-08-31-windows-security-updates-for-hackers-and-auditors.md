...
In this blog we will look at the process of understanding how Windows versioning works to obtaining the Windows version information and list of installed KBs from the local or a remote system, to obtaining an accurate view on the risk the Windows OS is exposed to, introducing various utilities on the way.
...
Once the Windows version information and installed security updates have been collected, the next step is to identify which security updates are missing. In case of an audit, the client to be audited can provide the resulting files while the evaluation of these files can be performed off their premises. Same applies for the hacker who can pull the files locally for further investigation to identify vulnerabilities and potential available exploits.
